...
class Spec extends SpecBase {
  ...
  testExc(eval("List(2, 3, 4, 5).map((x, y, z) => x + y)"), "arity mismatch")
  test(eval("1 :: 2 :: 1 :: Nil"), "List(1, 2, 1)")
  testExc(eval("1 :: 2"), "not a list")
  testExc(eval("List(42, fun).head"), "free identifier")
  testExc(eval("List(5, 7).flatMap(x => y => List(x, x * 2))"), "not a list")
  testExc(eval("List(3, 6, 7, 4, 2).filter(x => y => x % 2 == 0)"), "not a boolean")
  testExc(eval("List(3, 6, 7, 4, 2).filter((x, y) => x % 2 == 0)"), "arity mismatch")
  testExc(eval("List(3, 6, 7, 4, 2).filter(x % 2 == 0)"), "free identifier")
  test(eval("List(1, 2, 3).foldLeft(-1, (x, y) => x + y)"), "5")
  testExc(eval("List(1, 2, 3).foldLeft((x, y) => x + y)"), "parse error")
  testExc(eval("List(1, 2, 3).foldLeft(Nil, (x, y) => x + y)"), "invalid operation")
  test(eval("Nil.foldLeft(Nil, (x, y) => x + y)"), "List()")
  testExc(eval("List(1, 2, 3).foldLeft(0, (x) => x + y + z)"), "arity mismatch")
  testExc(eval("List(1, 2, 3).foldLeft(0, x + y + z)"), "free identifier")
  testExc(eval("List(1, 2, 3).foldLeft(0, 0)"), "not a function")
  test(eval("val x = 1; (1, 2, 3 + 1 + x)"), "(1, 2, 5)")
  test(eval("val x = 1; (1, 2, 3 + 1)"), "(1, 2, 4)")
  test(eval("(1, 2, 3 + 1)"), "(1, 2, 4)")
  testExc(eval("(1, 2, 3)._-1"), "parse error")
  testExc(eval("(1, 2, 3)._0"), "parse error")
  // Problem #1: interp
  // -------------------------------------------------------------------------
  test(eval("42"), "42")
  test(eval("true"), "true")
